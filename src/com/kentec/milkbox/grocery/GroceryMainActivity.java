...
//					+"or type in the blank.", "searchDialog");
			moderatorInteractiveDialog("请说出您要的商品"
					, indexNo
					,"请说出您要的商品",
					"searchDialog", 9);
			}
...
//						 + "<big>● Search</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>" ;
		...
		String dialogStr = "这是您可下的命令 :<sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				 + "<big>● 项目 1 到 10</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				 + "<big>● 查找</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				 + "<big>● 愿望清单</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				 + "<big>● 交易纪录</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				 + "<big>● 结账</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				 + "<big>● 主画面</big><sub>&nbsp;&nbsp;<small><small></small></small></sub><br>"
				  ;		
		moderatorDialog(dialogStr, indexNo + 2, 8000);
		
		// 唸出資訊
		ArrayList<String> speakStrs = new ArrayList<String>();
		speakStrs.add("您可以下的命令:");
...
//				"Which number of item do you want to browse?", "echoGroceryStoreCategory");
		MilkBoxTTS.speak("请选择您要的类别名称或编号", "echoGroceryStoreCategory");
	}
	...
	GrocerySearchAddToCartTask.OnSucessListener addProductSearchSuccessListener = new GrocerySearchAddToCartTask.OnSucessListener() {
		...
		public void success(ProductResult mProductResult) {
			...
			if(pList.size() > 0) {
				moderatorDialog( " ["+pList.get(0).getName()+"] 加入购物车内"
								, indexNo, 5000);
				MilkBoxTTS.speak("我已经帮您把 "+pList.get(0).getName()+" 加入购物车内"
	...
	GrocerySearchDeleteFromCartTask.OnSucessListener deleteProductSearchSuccessListener = new GrocerySearchDeleteFromCartTask.OnSucessListener() {
		...
		public void success(String productName) {
			int indexNo = (int) (Math.random() * 2) + 1;
			moderatorDialog( " ["+productName+"] 已从购物车删除"
					, indexNo, 5000);
			MilkBoxTTS.speak("我已经帮您把 "+productName+"  从购物车删除"
		...
		public void fail(int failCode) {
			int indexNo = (int) (Math.random() * 2) + 1;
			moderatorDialog( "它不在购物车里"
					, indexNo, 4000);
			MilkBoxTTS.speak("它不在购物车里"
					, "voicesearchdeletefromcart");
	...
	GroceryCategoryTask.OnSucessListener categorySuccessListener = new GroceryCategoryTask.OnSucessListener()
	{
		...
		public void success(final ArrayList<Category> cList)
		{
			...
			if(flagForGreetingCommand == true) {
				moderatorDialog( "欢迎来到生鲜购物,\n" +
						"共有 "+cList.size()+" 种类别供您选择,\n" +
						"请选择您要的类别名称或编号", indexNo, 9000);
...
//							"Please select a category or a number.", "echoGroceryStoreGretting");
				MilkBoxTTS.speak("欢迎来到生鲜购物," +
							"共有 "+cList.size()+" 种类别供您选择,"+
							"请选择您要的类别名称或编号", "echoGroceryStoreGretting");
				...
				addASRCMDAdapter(new ASRCMDAdapter(category)
				{
					...
					public void action()
					{
						if(groceryLevel==0) {
							...
							MilkBoxTTS.speak("没有这样东西", "echoGroceryNoTheseItem");
						}
					}
					...
					addASRCMDAdapter(new ASRCMDAdapter(category)
					{
						...
						public void action()
						{
							if(groceryLevel==1) {
								...
								MilkBoxTTS.speak("没有这项商品。", "echoGroceryNoTheseItem");							
							}
						}
